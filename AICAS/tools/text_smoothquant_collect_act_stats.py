...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Collect text-model SmoothQuant activation stats via llama-mtmd-profiler.")
    ...
    parser.add_argument("--output", default="AICAS/artifacts/text_sq_raw_act_stats.json")
...
def main() -> int:
    ...
    merged = {}
    tmp_dir_obj = tempfile.TemporaryDirectory(prefix="text-sq-act-stats-")
    tmp_dir = Path(tmp_dir_obj.name)
    try:
        for index, item in enumerate(samples, start=1):
            stats_path = tmp_dir / f"text_stats_{index:04d}.json"
            log_path = tmp_dir / f"text_stats_{index:04d}.log"
        ...
        out = {
            "schema": "aicas.llama.text.act_stats.v1",
        ...
        print(f"Wrote text activation stats: {output}")
