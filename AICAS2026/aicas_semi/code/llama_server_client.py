...
def _request(
    ...
    data = None
    headers = {}
    deadline = time.monotonic() + loading_wait_timeout if loading_wait_timeout > 0 else None
    ...
    while True:
        ...
        try:
            ...
            if deadline is not None and exc.code == 503 and time.monotonic() < deadline:
                if "Loading model" in body or '"type":"unavailable_error"' in body:
            ...
            if deadline is not None and time.monotonic() < deadline:
                if any(token in reason for token in ("Broken pipe", "Connection refused", "Connection reset")):
                    time.sleep(loading_retry_delay)
                    continue
