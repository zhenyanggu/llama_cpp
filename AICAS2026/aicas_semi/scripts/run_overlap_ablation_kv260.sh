...
usage() {
  cat <<'USAGE'
Usage:
  run_overlap_ablation_kv260.sh [options]

Runs the prefill two-level overlap ablation on KV260 and writes JSON for plotting.
...
Options:
  ...
  --shape-name <name>           Shape label
  --m <n>                       GEMM M (default: 768)
  --k <n>                       GEMM K (default: 2048)
  --n <n>                       GEMM N (default: 768)
...
RUN_ID="$(date +%Y%m%dT%H%M%S)-overlap-ablation"
OUTPUT_ROOT="$REPO_DIR/AICAS2026/aicas_semi/results/overlap_ablation"
REMOTE_ROOT="/home/ubuntu/overlap-ablation"
...
SHAPE_NAME="m5_ffn_down_768x2048x768"
M=768
K=2048
N=768
SKIP_BUILD=0
...
while [ $# -gt 0 ]; do
  case "$1" in
    ...
    --shape-name) SHAPE_NAME="$2"; shift 2 ;;
    --m) M="$2"; shift 2 ;;
    --k) K="$2"; shift 2 ;;
    --n) N="$2"; shift 2 ;;
...
build_binaries() {
  ...
  (
    ...
    make -C "$PREFILL_RUNTIME_DIR" \
      ...
      versa_p_prefill_overlap_ablation_test \
      -j1
  )
}
...
stage_remote() {
  ...
  scp "${SSH_OPTS[@]}" \
    ...
    "$PREFILL_RUNTIME_DIR/build/bin/versa_p_prefill_overlap_ablation_test" \
...
run_remote() {
  ...
  run_ssh "
    ...
    ./versa_p_prefill_overlap_ablation_test --name '$SHAPE_NAME' --m '$M' --k '$K' --n '$N' 2>&1 | tee '$REMOTE_RUN_DIR/logs/prefill_overlap_ablation.log'
  "
}

pull_and_analyze() {
  scp -r "${SSH_OPTS[@]}" "$TARGET:$REMOTE_RUN_DIR/logs/." "$LOCAL_RUN_DIR/"
  python3 "$SCRIPT_DIR/analyze_overlap_ablation.py" \
    --run-id "$RUN_ID" \
    --log "$LOCAL_RUN_DIR/prefill_overlap_ablation.log" \
    --overlay "$PREFILL_OVERLAY_APP" \
    --out-json "$LOCAL_RUN_DIR/prefill_overlap_ablation_summary.json" \
    --notes "$LOCAL_RUN_DIR/prefill_overlap_ablation_notes.md"
}
...
echo "Overlap ablation results: $LOCAL_RUN_DIR"
