...
usage() {
  cat <<'EOF'
Usage:
  run_layer_gemm_replay.sh [options] [-- <extra replay args>]

Extract one NPU layer tile shape from ggml_npu_profile.json, then run
kv260_layer_gemm_replay_test on KV260.

Options:
  --profile-json <path>      Profile json (default: AICAS2026/V2/results/official/20260416T075956Z-v2-throughput-profile-double-dma-only/ggml_npu_profile.json)
  ...
  --sdk-env <path>           SDK env script for cross build
  --skip-build               Reuse existing local binary
  -h, --help                 Show help

Examples:
  # replay first ffn_down tile with current defaults (double_mvin + bias path)
  bash AICAS2026/V2/scripts/run_layer_gemm_replay.sh

  # replay with no bias (for instruction-path isolation)
  bash AICAS2026/V2/scripts/run_layer_gemm_replay.sh -- --no-bias
EOF
}

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
PROFILE_JSON="$ROOT_DIR/AICAS2026/V2/results/official/20260416T075956Z-v2-throughput-profile-double-dma-only/ggml_npu_profile.json"
LAYER_ID=""
TILE_INDEX="0"
HOST="192.168.0.10"
USER_NAME="ubuntu"
...
if [ "$SKIP_BUILD" -ne 1 ]; then
  [ -f "$SDK_ENV" ] || { echo "Missing SDK env: $SDK_ENV" >&2; exit 1; }
  # SDK env requires LD_LIBRARY_PATH unset before sourcing.
  unset LD_LIBRARY_PATH || true
  # shellcheck disable=SC1090
  source "$SDK_ENV"
  make -C "$ROOT_DIR/npuruntime/kv260/runtime" kv260_layer_gemm_replay_test
fi

LOCAL_BIN="$ROOT_DIR/npuruntime/kv260/runtime/kv260_layer_gemm_replay_test"
...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR "$TARGET" "mkdir -p '$REMOTE_DIR'"
scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR "$LOCAL_BIN" "$TARGET:$REMOTE_BIN"
...
ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR "$TARGET" "$REMOTE_CMD_STR"
